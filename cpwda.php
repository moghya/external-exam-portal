<?php
	...
	if(!isloggedins()&&!isloggedint()&&isloggedina())
	{
		if(isset($_POST['cpwd'])&&isset($_POST['npwd'])&&isset($_POST['cnpwd']))
		{
			...
			{
				$cpwdhash = md5($cpwd);
				$query = "SELECT * FROM `adm` WHERE `id`='".$_SESSION['id']."' AND `pwd`='".$cpwdhash."'";
				...
				if($query_run = mysql_query($query))
				{
					if(mysql_num_rows($query_run)==1)
					{
						...
						{
							$npwdhash = md5($npwd);
							$queryup = "UPDATE `adm` SET `pwd`='".$npwdhash."' WHERE `id` = '".$_SESSION['id']."'";
...
<nav class = "navbar navbar-default" role = "navigation">
   ...
   <div class = "collapse navbar-collapse" id = "nav">	
		...
		<li class="active"><a href='cpwda.php'>Change Password</a></li>
